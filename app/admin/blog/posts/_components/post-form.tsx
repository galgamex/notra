...
import { CreatePostFormValues, CategoryWithDetails, TagWithDetails, PostWithDetails } from '@/types/blog';
...
export default function PostForm({ mode, postId }: PostFormProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const promises = [
          fetch('/api/blog/categories').then(res => res.ok ? res.json() : []),
        ...
        if (mode === 'edit' && postId) {
          promises.push(
            fetch(`/api/blog/posts/${postId}`).then(res => res.ok ? res.json() : null)
          );
        }
  ...
  const handleTagInput = async (value: string) => {
    ...
    // 检查是否包含逗号，如果有则处理标签
    if (value.includes(',')) {
      const tagNames = value.split(',').map(name => name.trim()).filter(name => name.length > 0);
  ...
  const handleSubmit = async (status: PostStatus) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.error || `${mode === 'create' ? '创建' : '更新'}文章失败`);
      }
