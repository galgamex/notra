...
const websiteEditFormSchema = z.object({
	id: z.string(),
	name: z
		.string()
		.min(1, '网站名称不能为空')
...
export default function WebsiteEditForm({ website }: WebsiteEditFormProps) {
	...
	const form = useForm<UpdateWebsiteFormValues>({
		resolver: zodResolver(websiteEditFormSchema),
		defaultValues: {
			id: website.id,
			name: website.name || '',
			url: website.url || '',
			...
			tagIds: website.tags?.map((tag) => tag.tag.id) || [],
			isRecommend: website.isRecommend || false,
			isFeatured: website.isFeatured || false,
			isNSFW: website.isNSFW || false,
	...
	const onSubmit = async (values: UpdateWebsiteFormValues) => {
		...
		try {
			...
			if (response.ok) {
				...
				toast.error(error.error || '更新网站失败');
			}
		} catch (error) {
